...
st.set_page_config(page_title="Mueblería Pro v35", layout="wide")
...
if choice == "Nuevo Proyecto":
    ...
    cli_l = sorted(pd.read_sql("SELECT DISTINCT cliente FROM proyectos", conn)['cliente'].tolist())
    sup_l = sorted(pd.read_sql("SELECT DISTINCT suplidor FROM proyectos", conn)['suplidor'].tolist())
    with st.form("f_n", clear_on_submit=True):
        f = st.date_input("Fecha", date.today())
        c1, c2 = st.columns(2)
        cl_s = c1.selectbox("Cliente", ["+ Nuevo"] + cli_l)
        cl_f = c1.text_input("Nombre Cliente").upper() if cl_s == "+ Nuevo" else cl_s
        su_s = c2.selectbox("Suplidor", ["+ Nuevo"] + sup_l)
        ...
        if st.form_submit_button("Guardar"):
            c.execute("INSERT INTO proyectos (fecha_creacion,cliente,mueble,suplidor,precio_venta,costo_fabrica,adelanto_cliente,adelanto_suplidor,estado) VALUES (?,?,?,?,?,?,?,?,?)",(f.strftime("%Y-%m-%d"), cl_f, mu, su_f, p_v, c_f, 0.0, 0.0, "En Proceso"))
    ...
    if not df_p.empty:
        with st.form("f_p"):
            ...
            if st.form_submit_button("Registrar"):
                ...
                c.execute("INSERT INTO historial_pagos (proyecto_id, fecha, tipo_movimiento, monto) VALUES (?,?,?,?)", (id_p, date.today().isoformat(), t, m))
                conn.commit(); st.rerun()

elif choice == "✏️ Corregir Datos":
    st.header("✏️ Editor")
    t1, t2 = st.tabs(["Proyectos", "Pagos"])
    with t1:
        df_pro = pd.read_sql("SELECT * FROM proyectos", conn)
        if not df_pro.empty:
            sel_p = st.selectbox("Editar:", [f"ID {r['id']} - {r['cliente']}" for _, r in df_pro.iterrows()])
            ...
            with st.form("e_p"):
                ...
                if st.form_submit_button("Actualizar"):
                    c.execute("UPDATE proyectos SET cliente=?, suplidor=?, precio_venta=?, costo_fabrica=?, estado=? WHERE id=?", (nc.upper(), ns.upper(), nv, ncost, nest, id_p))
                    conn.commit(); st.rerun()
    with t2:
        ...
        if not df_hi.empty:
            sel_h = st.selectbox("Borrar Pago:", [f"ID {r['id']} | {r['cliente']} | ${r['monto']}" for _, r in df_hi.iterrows()])
            id_h = int(sel_h.split(" ")[1])
            h = df_hi[df_hi['id'] == id_h].iloc[0]
            if st.button("🗑️ Confirmar Borrar"):
    ...
    if st.button("📥 Descargar Excel"):
        ...
        with pd.ExcelWriter(out, engine='xlsxwriter') as wr:
            pd.read_sql("SELECT * FROM proyectos", conn).to_excel(wr, sheet_name='Proyectos', index=False)
            pd.read_sql("SELECT * FROM historial_pagos", conn).to_excel(wr, sheet_name='Pagos', index=False)
        st.download_button("Descargar", out.getvalue(), f"Respaldo_{date.today()}.xlsx")
    st.divider()
    df_p = pd.read_sql("SELECT * FROM proyectos", conn)
    if not df_p.empty:
        for col in ['precio_venta','costo_fabrica','adelanto_cliente','adelanto_suplidor']:
        ...
        c_cl, c_su = st.columns(2)
        c_cl.subheader("💰 Clientes"); c_cl.table(df_p[df_p['Por Cobrar']>0][['cliente','Por Cobrar']].style.format("${:,.2f}"))
        c_su.subheader("🏭 Suplidores"); c_su.table(df_p[df_p['Por Pagar']>0][['suplidor','Por Pagar']].style.format("${:,.2f}"))
